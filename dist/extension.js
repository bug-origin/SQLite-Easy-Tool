...
var SqliteEditorProvider = class {
  constructor(context) {
  ...
  async resolveCustomEditor(document2, webviewPanel) {
    webviewPanel.webview.options = { enableScripts: true };
    const dbPath = document2.uri.fsPath;
    let db = null;
    const saveDb = () => {
      if (db) {
    ...
    try {
      ...
      const SQL = await (0, import_sql.default)({ locateFile: () => wasmPath });
